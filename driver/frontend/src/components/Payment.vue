...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-text style = "margin-left:-15px; margin-top:10px;">
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field type="number" label="MatchId" v-model="value.matchId"/>
          </div>
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="PaymentAction" v-model="value.paymentAction"/>
          </div>
          <div class="grey--text ml-4" v-else>
            PaymentAction :  {{value.paymentAction }}
          </div>
    ...
    methods: {
      ...
      async save(){
        try{
          ...
          if(this.isNew){
            temp = await axios.post(axios.backend + '/payments', this.value)
          }else{
